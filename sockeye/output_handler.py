...
def get_output_handler(output_type: str,
    ...
    elif output_type == C.OUTPUT_HANDLER_TRANSLATION_NEMATUS:
        return NematusStringWithAlignmentsOutputHandler(output_stream)
...
class NematusStringWithAlignmentsOutputHandler(StringOutputHandler):
    """
    Output handler to write translations and alignments to a stream. The format is compatible with the output
    generated by Nematus. Note that unlike other output handlers each input sentence will result in an output
    ...
    def handle(self,
        ...
        :param t_walltime: Total wall-clock time for translation.
        """
        self.stream.write("%d ||| %s ||| %f ||| %s ||| %d ||| %d\n" % (t_input.id,
                                                                       " ".join(t_input.tokens),
                                                                       t_output.score,
                                                                       " ".join(t_output.tokens),
                                                                       len(t_input.tokens),
                                                                       len(t_output.tokens)))
        attention_matrix = t_output.attention_matrix
        for t in range(0, attention_matrix.shape[0]):
            attention_vector = attention_matrix[t]
